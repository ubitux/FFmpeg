...
static int graph_insert_fifos(AVFilterGraph *graph, AVClass *log_ctx)
{
    ...
    for (i = 0; i < graph->nb_filters; i++) {
        ...
        for (j = 0; j < f->nb_inputs; j++) {
            ...
            switch (f->inputs[j]->type) {
            case AVMEDIA_TYPE_VIDEO:    fifo = avfilter_get_by_name("fifo");  break;
            case AVMEDIA_TYPE_AUDIO:    fifo = avfilter_get_by_name("afifo"); break;
            case AVMEDIA_TYPE_SUBTITLE: fifo = avfilter_get_by_name("sfifo"); break;
            default:
                av_assert0(0);
            }
