...
static int64_t luma_abs_diff(const FieldMatchContext *fm,
                             const AVFrame *f1, const AVFrame *f2)
{
    ...
    av_pixelutils_sad_fn sadfn = fm->sad;
    const int bsize = fm->block_size;
    int64_t acc = 0;

    for (y = 0; y < height - bsize + 1; y += bsize) {
        for (x = 0; x < width - bsize + 1; x += bsize)
            acc += sadfn(srcp1 + x, src1_linesize,
                         srcp2 + x, src2_linesize);
        srcp1 += bsize * src1_linesize;
        srcp2 += bsize * src2_linesize;
    }
    emms_c();
    return acc;
}
